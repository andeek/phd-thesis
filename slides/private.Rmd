...
- $\tilde{\boldsymbol X} = \{H^{(1)}_1, \dots, H^{(1)}_{N_{(H,1)}}, \dots, H^{(M)}_1, \dots, H^{(M)}_{N_{(H,M)}}, \boldsymbol X\}$ 
    - Hidden variables $\{ H_{i}^{(j)} : i=1,\ldots, N_{(H,j)}, j=1,\ldots,M\}$ 
    - Visible variables $\boldsymbol X =(X_1,\ldots,X_N)$
    - $H_{i}^{(j)}, X_k \in \mathcal{X} = \{-1,1\}$ for all $i, j, k$
...
- Let $\boldsymbol \theta_N = (\boldsymbol \alpha^{(1)}, \dots, \boldsymbol \alpha^{(M)}, \boldsymbol \beta,\Gamma^{(0)}, \dots, \Gamma^{(M - 1)}) \in \Theta_N \subset \mathbb{R}^{q(N)}$, 
    \footnotesize
    $$
...
- The visible DBM model specification is an example of a \blue{FSFS model}.
...
\vspace{.1in}
    - A "weight" parameter is placed on each interaction between visibles, $X_1, \dots, X_N$ and the first layer of latent variables, $H^{(1)}_1, \dots, H^{(1)}_{N_{(H,1)}}$
\vspace{.1in}    
    - The number $q(N)$ of components in the parameter vector is dependent on the dimension of the visibles
\vspace{.1in}    
- For visibles $X_1, \dots, X_N$ with support $\mathcal{X}^N$, a DBN is also a \blue{FSFS model}
