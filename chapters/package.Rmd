...
Markov random field (MRF) models are common for spatial data (as well as graph, network and other data structures). Rather than specifying a joint distribution directly, a  model is specified through a set of full conditional distributions for the spatials location. The `R` [@r-lang] package `conclique` provides a new, fast way to simulate from a MRF model using a Gibbs sampler and implements a formal goodness-of-fit (GOF) test for model assessment [@kaiser2012goodness].

Throughout this paper, we will employ the following notational conventions: see also @kaiser2000construction for spatial MRF specifications. Spatial locations are denoted $\boldsymbol s_i$ where the set of all locations $\{\boldsymbol s_i: i = 1, \dots, n\}$ form a regular lattice. The values of random variables at these locations will be denoted $\{ Y(\boldsymbol s_i): i=1, \dots, n \}$ and spatial neighborhoods, $\mathcal{N}_i$ will be pre-specified for each location (e.g., four-nearest or eight-nearest neighborhoods). We then denote the neighboring values as $\boldsymbol y(\mathcal{N}_i) = \{ y(\boldsymbol s_j) : \boldsymbol s_j \in \mathcal{N}_i \}$ and define the full conditional distributions for each spatial location as $\{ f_i(y(\boldsymbol s_i)|\boldsymbol y(\mathcal{N}_i), \boldsymbol \theta): i=1, \dots, n \}$, where each $f_i(y(\boldsymbol s_i)|\boldsymbol y(\mathcal{N}_i), \boldsymbol \theta)$ is conditional a pmf/pdf of $Y(\boldsymbol s_i)$ given values for its neighbors $\boldsymbol y(\mathcal{N}_i)$. Lastly, we let $F_i(y(\boldsymbol s_i)|\boldsymbol y(\mathcal{N}_i), \boldsymbol \theta)$ be the conditional cdf of $Y(\boldsymbol s_i)$ given values for its neighbors $\boldsymbol y(\mathcal{N}_i)$ and a potential set $\boldsymbol \theta$ of parameter values. For simplicity, we will assume a common conditional form ($f_i=f$ and $F_i = F$) for all locations, which occurs often in practice. 

In the following sections, we introduce the `conclique` package through demonstration of simulation from a particular MRF model as well as an associated simulation-based test for assessing GOF of spatial MRF models. Currently `conclique` can be installed via GitHub using the following `R` code.
...
\end{array},
$$
with $*$'s above as the positions of neighbors. For a four-nearest neighbor model, the set of maximal concliques has $Q = 2$, where Figure \ref{fig:conc} displays the resulting concliques for the $20\times 20$ example. For comparison, an eight-nearest neighbor model will yield a set of maximal concliques containing $Q = 4$ concliques.

```{r concliques, eval=FALSE, echo=TRUE}
library(conclique)
...
```{r conc-plots, echo=FALSE, fig.cap="\\label{fig:conc}Minimal conclique cover for a given lattice of size $20\\times20$ using a four-nearest neighbors structure wrapped on a torus. This results in 2 concliques.", fig.width=3, echo=FALSE}
...
1. For $i = 1, \dots, M$,
    ...
    1. Set $\ell =2$. Considering all $\boldsymbol s_j \in \mathcal{C}_\ell$, sample $\{Y^{(i)}(\boldsymbol s_j): \boldsymbol s_j \in \mathcal{C}_\ell\}$ by independently drawing $Y^{(i)}(\boldsymbol s_j) \sim F(\cdot|\{Y^{(i)}(\boldsymbol s), \boldsymbol s \in N_j \cap \mathcal{C}_k \text{ where } \ell < k\}, \{Y^{(i - 1)}(\boldsymbol s), \boldsymbol s \in N_j \cap \mathcal{C}_k \text{ where } \ell > k\})$
...
$$
  f_i(y(\boldsymbol s_i)|\boldsymbol y(\mathcal{N}_i),\kappa,\eta,\rho) = \frac{1}{\sqrt{2 \pi} \rho}\exp\left( -\frac{[y(\boldsymbol s_i) - \mu(\boldsymbol s_i) ]^2}{2 \rho^2}\right)
$$
with conditional mean
$$
\mu(\boldsymbol s_i) = \kappa + \eta \sum_{\boldsymbol s_j \in \mathcal{N}_i}[y(\boldsymbol s_j)-\kappa].
$$
where $\rho^2$ is the conditional variance for each location, $\eta$ represents a dependence parameter and $\kappa$ represents a location parameter specifying the (unconditional) mean of each location [@kaiser2012centered]. As an example, we will simulate from the Gaussian MRF with $\rho^2 = 2, \kappa = 20, \eta = 0.24$ using both a conclique-based Gibbs sampler and a sequential Gibbs sampler to compare run time for $10,000$ iterations of the sampler generating data on a $20\times 20$ lattice.

```{r concliques-contd, echo=TRUE}
...
sequential_simulation <- run_sequential_gibbs(neighbors, inits, 
                                              "gaussian_single_param", 
                                              params, num_sample)
```
...
where $F(y|y(\mathcal{N}_i),\boldsymbol \theta) \equiv F_i(y|y(\mathcal{N}_i), \boldsymbol\theta)$ denotes the conditional cdf of observation $Y(s_i)$, assumed to have a common form $F$ for all locations. Similar residuals, $R(\boldsymbol s_i)$, can be defined for non-continuous $F$ too. It then holds that *within* a conclique, the generalized spatial residuals are iid Uniform$(0, 1)$-distributed if $F(\cdot | \cdot)$ corresponds to the true form of an underlying conditional distribution.

The spatial residuals are obtainable using the `conclique` package. We can continue with our example from Section \ref{concliques} using spatial data on a $20 \times 20$ grid from a four-nearest neighbor Gaussian MRF with $\rho^2 = 2, \kappa = 20, \eta = 0.24$. To demonstrate the use of generalized spatial residuals as a fit assessment tool, we generate data from the true model and compute residuals with the true model parameters as well as misspecified parameters, say one with $\eta = -0.10$. We compare the empirical cdf of the generalized spatial residuals within each conclique, $\{R(\boldsymbol s_i): \boldsymbol s_i \in \mathcal{C}_j\}_{j = 1}^Q$ to the cdf of a Uniform$(0,1)$, where $Q = 2$ in our example with a four-nearest neighborhood. The results are in Figure \ref{fig:compare}.
...
```{r resids-plot, echo=FALSE, fig.height=3.2, fig.show='hold', out.width="50%", fig.cap='\\label{fig:compare}Looking at the emprirical cdf of generalized spatial residuals from simulated Gaussian MRF with $\\rho^2 = 2, \\kappa = 10, \\eta = 0.24$ versus the correct model (left) and an incorrect model with $\\eta = -0.10$ (right). The black line represents the cdf of a Uniform$(0,1)$.'}
##good model ------
...
Under the incorrect model, we can see that, for both for both concliques, all of the residuals are below the Uniform$(0,1)$ cdf, where the departure of uniformity suggests a poor model fit (which, in fact, is by design from using a misspecified model).
...
Both the Kolmogorov-Smirnov statistic and the Cramèr-von-Mises criterion are implemented in `conclique`, and any user specified aggregation can be supplied; `mean` and `max` are reasonable choices. In this example, we employ a maximum across concliques of the Kolmogorov-Smirnov statistic.
...
We can see that the incorrectly specified model has a higher value of the statistic, which points to potentially rejecting this model. However without a reference distribution, it becomes impossible to judge. As a remedy, we have the ability to generate spatial data from a model using our conclique-based Gibbs simulation functions in `conclique` and thereby simulate (or parametrically bootstrap) a reference distribution for conclique-based test statistics.

### Getting a reference distribution

In order to answer the question, "Could my data have come from my fitted model?", we will employ a GOF test using the framework laid out in Sections \ref{generalized-spatial-residuals} and \ref{test-statistics} and use a parametric bootstrap (implemented via the conclique-based Gibbs sampler from Section \ref{simulation}) to obtain the reference distribution for our statistic. Prior to using the conclique-based Gibbs sampler, we fit the model using a pseudo-likelihood approach [@besag1974spatial] and obtained estimates for $\eta, \kappa,$ and $\rho^2$ to the data generated from a four-nearest neighbor Gaussian MRF with $\rho^2 = 2, \kappa = 10, \eta = 0.24$. These estimates can be found in Table \ref{tab:ests}.
...
data.frame(eta = mple$eta,
  ...
  rename(`$\\hat{\\eta}$` = eta,
         `$\\hat{\\kappa}$` = kappa,
         `$\\hat{\\rho}^2$` = rho) %>%
  xtable(caption = "\\label{tab:ests}Maximum pseudo-likelihood estimates for $\\eta, \\kappa,$ and $\\rho^2$ for data generated from a four-nearest neighbor Gaussian MRF with $\\rho^2 = 2, \\kappa = 10, \\eta = .24$.") %>%
...
From the reference distribution generated by the parametric bootstrap within `conclique`, the test statistic is $`r round(gof_bs$t, 4)`$ with a p-value of $`r round(gof_bs$p.value, 4)`$. For the original Gaussian model fit to normal data, we fail to reject the null hypothesis, meaning the four-nearest neighbor Gaussian model is not an obviously inadequate one for these data. We should expect this result because this exercise fit the known model family to the data (Gaussian MRF with one dependence parameter and a four-nearest neighbor structure). For contrast, fitting this same Gaussian MRF model with four-nearest neighbors to data generated from a spatial lognormal model gives a p-value of $`r round(gof_bs_lognormal$p.value, 4)`$ from the same conclique-based GOF procedure, indicating the GOF method detects this departure in distributional class. However, through the use of `conclique`, there are many distributions possible, and with the ability to extend both model distribution family (e.g., non-Gaussian) and assumed neighborhood structure, the prospects are numerous.

## Extending `conclique`

For simulating from a Gaussian MRF (e.g., with four-nearest neighbor structure), there are other potential simulation options available, such as determining the joint multivariate normal distribution for the data from specified conditional Gaussian distributions (i.e. finding the spatial corvariance matrix for the observations) and considering direct simulation; see @cressie1993statistics One of the key advantages to using conclique-based approaches for simulation, as well as GOF tests, is the ability to consider non-Gaussian conditional models that go beyond a four-nearest neighbor structure.
...
This function should return a sampled data set the same size as the original data. For example, the sampler function for the single dependency parameter Gaussian model is reproduced below.
...
In order to extend `conclique` to use an arbitrary conditional distribution for the generalized spatial residuals, the user must specify a cdf function which takes as parameters the same parameters as the sampler function from Section \ref{conditional-distribution}, `data` and `params`.
...
`conclique` is a fast and flexible implementation of a conclique-based method for simulating MRF data and performing associated GOF tests. By employing a conclique-based sampling method, many spatial models become feasible (like Bernoulli, Poisson, truncated Gamma with one or multiple dependence parameters). Through the clever use of conditional independence within a conclique [@kaiser2012goodness] and an `Rcpp` back end [@rcpp], this method achieves improved speed over traditional spatial Gibbs sampling methods, allowing for a broad range of applications.
