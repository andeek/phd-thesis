...
Additionally, S-unstable FSFS model sequences are connected to degenerate models, where model *degeneracy* implies placing all probability on a small portion of the sample space. For perspective, note that differing bounds on $1/N\cdot\text{ELPR}(\boldsymbol \theta_N)$ in (\@ref(eq:elpr)) provide a spectrum of levels of "stability" and Proposition \@ref(prp:instab-elpr) indicates increasing and undesirable sensitivity of model probabilities (i.e., for one component changes in outcomes) as (\@ref(eq:elpr)) increases. Furthermore, as the instability measure (\@ref(eq:elpr)) grows, FSFS model sequences are guaranteed to slide into full degeneracy as Proposition \@ref(prp:degenFSFS) will show. Define a $\epsilon$-modal set 
$$
